...
prob_win = zeros(M,size(ind_samples,2));
Sum = 0;
...
for P1=1:M
   for P2=1:M
       ...
       prob_win(P1, l) = normcdf(ind_samples(P1,l) - ind_samples(P2,l));
       end
   end
end
...
[kk_g, ii_g] = sort(mean_prob, 'descend');
...
axis([0 1 0.5 np+0.5])
title('Gibbs Sampling', 'FontSize', 20, 'FontWeight', 'bold')
xlabel('Average Probability of Winning against other Players', 'FontSize', 15, 'FontWeight','bold');
ylabel('Player Name', 'FontSize', 15, 'FontWeight','bold');
